...
class ReadMitEcg:
    ...
    def nextbatch_limitperclasssize(self, batchsize, classsize):
        self.currentReadRecord = 0 #np.random.randint(len(self.filenames) / 3 - 1 - 1)
        batch_xs = []
        batch_ys = []
        lenperclass = [0] * len(self.heartBeatTypeLearn)
        for i in range(len(lenperclass)):
            lenperclass[i] = 0
        for b_i in range(batchsize):
            data = np.fromfile(self.filenames[(self.currentReadRecord) * 3 + 1], dtype=np.uint8)
            head = open(self.filenames[(self.currentReadRecord) * 3 + 2], "r")
            ...
            M2H = np.right_shift(npdata[:, 1], np.full(npdata[:, 1].size, 4))
            M1H = np.bitwise_and(npdata[:, 1], np.full(npdata[:, 1].size, 15))
            PRL = np.left_shift(np.bitwise_and(npdata[:, 1], np.full(npdata[:, 1].size, 8)),
                                np.full(npdata[:, 1].size, 9))
            PRR = np.left_shift(np.bitwise_and(npdata[:, 1], np.full(npdata[:, 1].size, 128)),
                                np.full(npdata[:, 1].size, 5))
            M = np.array([np.left_shift(M1H, np.full(M1H.size, 8)) + npdata[:, 0] - PRL,
                          np.left_shift(M2H, np.full(M2H.size, 8)) + npdata[:, 2] - PRR])
            M = np.transpose(M)
            ...
            while j < (len(ATRTIMED) - 1):
                if (ANNOTD[j] in self.heartBeatTypeLearn):
                    if (lenperclass[self.heartBeatTypeLearn.index(ANNOTD[j])] < classsize):
                        indoneperiod = np.where((TIME >= ATRTIMED[j]) & (TIME < ATRTIMED[j + 1]))
                        heartBeatVoltage = M[:, 0][indoneperiod]
                        heartBeatTime = TIME[indoneperiod]
                        scaledSampleInterval = (ATRTIMED[j + 1] - ATRTIMED[j]) / self.SCALEDSAMPLES
                        ...
                        if (self.one_hot):
                            ...
                            heartBeatLabel[self.heartBeatTypeLearn.index(ANNOTD[j])] = 1
                        batch_ys.append(heartBeatLabel)
                        lenperclass[self.heartBeatTypeLearn.index(ANNOTD[j])] = lenperclass[self.heartBeatTypeLearn.index(ANNOTD[j])] + 1
                        #print(lenperclass[self.heartBeatTypeLearn.index(ANNOTD[j])])
                j = j + 1
        return np.array(batch_xs), np.array(batch_ys)
